...
/* OTP Inputs & 6-Box Orbit Animation System */
.otp-inputs-wrapper {
  display: flex;
  justify-content: center;
  align-items: center;
...
/* 1. Typing bounce on individual box */
@keyframes otpBoxBounce {
  0% { transform: translateY(0) scale(1); }
  45% { transform: translateY(-4px) scale(1.09); border-color: #EAB308; box-shadow: 0 4px 12px rgba(234, 179, 8, 0.28); }
  100% { transform: translateY(0) scale(1); }
}

.otp-box.input-bounce {
  animation: otpBoxBounce 0.24s cubic-bezier(0.34, 1.56, 0.64, 1);
}
...
.otp-success-icon {
  ...
  align-items: center;
  justify-content: center;
  animation: successCheckDraw 0.4s ease-out 0.15s both;
}
...
.otp-success-text {
  ...
  white-space: nowrap;
  animation: successFadeUp 0.4s ease-out 0.15s both;
}
...
/* 4. Error State: Shake and return to horizontal */
...
.otp-inputs-wrapper.is-error .otp-box {
  border-color: #EF4444;
  background: #FEF2F2;
  transition: transform 0.35s cubic-bezier(0.34, 1.56, 0.64, 1), border-color 0.2s, background 0.2s;
}
